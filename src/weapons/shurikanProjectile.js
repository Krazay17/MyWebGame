...
export default class ShurikanProjectile extends WeaponProjectile {
    constructor(scene, x, y, player, doesChain = false) {
        super(scene, x, y, 'shurikan', player, 1);
        this.maxTargets = 2;
        this.doesChain = doesChain;
    ...
    enemyHit(enemy) {
        ...
        for (let i = 0; i < Math.min(validTargets.length, this.maxTargets); i++) {
            ...
            const chainShurikan = new ShurikanProjectile(this.scene, thisPos.x, thisPos.y, this.player)
